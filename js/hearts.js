...
// Continuous audio across pages
(function(){
if(!document.querySelector('audio')){const a=document.createElement('audio');a.id='bg-music';a.loop=true;
a.src='audio/song.mp3';document.body.appendChild(a);}
const a=document.querySelector('audio');
a.loop=true;
const saved=parseFloat(localStorage.getItem('musicTime')||0);
function startMusic(){a.currentTime=saved;a.play().catch(()=>{});}
a.addEventListener('loadedmetadata',()=>{a.currentTime=saved;},{ once:true });
a.play().catch(()=>{});
// Save position continuously
setInterval(()=>{if(!a.paused)localStorage.setItem('musicTime',a.currentTime);},200);
// Handle autoplay block - resume on first click
document.addEventListener('click',()=>{ a.currentTime=parseFloat(localStorage.getItem('musicTime')||0);a.play();},{once:true});
})();
